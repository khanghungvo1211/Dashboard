...
def download_predictions(n_clicks, selected_stock):
    ...
    # Determine train-test split based on 8-year training period
    ...
    return dcc.send_data_frame(df_download.to_csv, f'{selected_stock}_XGBoost_predictions.csv', index=False)
...
def create_layout(app):
    # Load dữ liệu từ file CSV cho mô hình XGBoost
    df = pd.read_csv('Result/forecast_summary_XGBoost.csv')

    layout = html.Div([
        html.H2('XGBoost Model Dashboard'),
        dcc.Graph(
            id='graph-xg',
            figure={
                'data': [
