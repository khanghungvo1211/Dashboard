...
if os.path.exists(forecast_summary_file):
    ...
    logging.info("Loaded 'forecast_summary_lstm_svr.csv' successfully.")
else:
    forecast_summary_df = pd.DataFrame()
    logging.warning("Could not find 'forecast_summary_lstm_svr.csv'. Evaluation metrics and forecasts will not be displayed.")
...
# Define app layout
app.layout = html.Div([
    html.H1('Stock Price Predictions Dashboard - LSTM_GRU'),
    ...
    html.Div([
        ...
        html.Div([
            ...
            dcc.DatePickerRange(
                ...
                max_date_allowed='2025-09-18',  # Extended by 1 year for forecast
                start_date='2014-09-18',        # Default start date
                end_date='2025-09-18'           # Extended by 1 year for forecast
            )
...
def update_graph(selected_stock, start_date, end_date, forecast_option):
    try:
        ...
        logging.info(f"Selected date range: {start_date.date()} to {end_date.date()}")
...
def download_predictions(n_clicks, selected_stock):
    try:
        ...
        total_samples = len(dates) - time_step  # Ensure time_step is defined
        time_step = 60  # Define time_step before using it
        ...
        return dcc.send_data_frame(df_download.to_csv, f'{selected_stock}_LSTM_SVR_Predictions.csv', index=False)
    except Exception as e:
